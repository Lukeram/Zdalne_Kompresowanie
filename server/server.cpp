...
int main(int argc, char** argv)
{
    ...
    while(1)
    {
        ...
        if(fork() == 0)
        {
                ...
                close(fd); //zamknij deskryptor gniazda
                int pid = getpid();
                string path = to_string(pid);
                mkdir(path.c_str(), 0777);
                recvdirectory(nfd,path);
                string command = "zip -r " + path + ".zip " + path;
                system(command.c_str());
                command = "rm -rf " + path;
                system(command.c_str());
                path += ".zip";
                if(!is_regular(path.c_str()))//jeżeli katalog
                {
                    if(senddirectory(nfd, path))
                    {
